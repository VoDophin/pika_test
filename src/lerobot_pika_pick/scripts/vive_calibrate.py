...
def main() -> None:
    """Force a fresh Vive/lighthouse calibration and stream tracker poses."""
    argv = sys.argv.copy()
    ...
    config_path = os.path.join(
        os.path.expanduser('~'), '.config', 'libsurvive', 'config.json'
    )
    ...
    while ctx.Running():
        ...
        if updated:
            ...
            if hasattr(pysurvive, "simple_serial_number"):
                serial_number = pysurvive.simple_serial_number(updated.ptr).decode(
                    "utf-8"
                )
